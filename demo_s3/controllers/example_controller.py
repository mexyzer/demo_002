...
class ExampleController(http.Controller):
    ...
    def get_examples(self, page=1, page_size=10):
        ...
        limit = page_size
        examples = request.env['demo.s3.example'].sudo().search([], limit=limit, offset=offset)
        total_count = request.env['demo.s3.example'].sudo().search_count([])
    ...
    def get_example(self, example_id):
        example = request.env['demo.s3.example'].sudo().browse(example_id)
    ...
    def create_example(self):
        ...
        new_example = request.env['demo.s3.example'].sudo().create({
            'name': data.get('name'),
